        ...
        <>
          ...
          <ul className="space-y-4">
            {candidates.map((candidate) => (
              <li key={candidate.id} className="bg-gray-700 p-4 rounded-lg shadow flex justify-between items-center">
                <div>
                ...
                <button
                  ...
                  className="px-6 py-2 bg-blue-500 hover:bg-blue-600 rounded-lg text-white font-semibold transition duration-150 ease-in-out disabled:opacity-50 disabled:cursor-not-allowed"
                >
          ...
          {!voterInfo.authorized && <p className="text-sm text-yellow-300 mt-4">You must be authorized to vote.</p>}
          {voterInfo.voted && <p className="text-sm text-blue-300 mt-4">You have already voted in this election.</p>}
          {!electionActive && <p className="text-sm text-red-300 mt-4">Voting is not currently active for this election.</p>}
        </>
      )}
    </div>
  );
