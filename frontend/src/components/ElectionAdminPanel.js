...
function ElectionAdminPanel({
  ...
  return (
    ...
    <div className="election-admin-panel p-4 sm:p-5 bg-gray-700 rounded-lg shadow-md mb-8">
      {selectedElectionId === 0 || !currentElectionDetails ? (
        ...
        <>
          <h3 className="text-lg sm:text-xl font-semibold mb-3 text-white">Managing Selected Election:</h3>
          <p className="text-base sm:text-lg text-sky-300 mb-2">
          ...
          <p className="text-xs sm:text-sm text-gray-400">
            Admin: <span className="font-mono text-xs sm:text-sm text-gray-300 break-all">{currentElectionDetails.admin}</span>
            <br />
          ...
          <div className="mt-4 space-y-4">
            ...
            <section>
              <h4 className="text-base sm:text-lg font-medium mb-2 text-white">Add New Candidate</h4>
              <form onSubmit={handleAddCandidate} className="flex flex-col sm:flex-row gap-2 sm:gap-3">
                <input
                  type="text"
                  value={newCandidateName}
                  onChange={(e) => setNewCandidateName(e.target.value)}
                  placeholder={`Candidate name for Election ID ${selectedElectionId}`}
                  className="flex-grow p-2 sm:p-3 bg-gray-900 border border-gray-600 rounded-lg focus:ring-2 focus:ring-sky-500 focus:border-sky-500 outline-none text-white text-sm sm:text-base"
                  required
                />
                <button
                  type="submit"
                  ...
                  className="px-4 py-2 sm:px-6 sm:py-3 bg-green-500 hover:bg-green-600 rounded-lg text-white font-semibold transition duration-150 ease-in-out disabled:opacity-50 disabled:cursor-not-allowed text-sm sm:text-base"
                >
                  Add Candidate
                </button>
              </form>
              {(currentElectionDetails.isActive || currentElectionDetails.isCompleted) &&
                <p className="text-xs text-red-300 mt-2">Cannot add candidates to an active or completed election.</p>}
              {currentElectionDetails.admin.toLowerCase() !== account.toLowerCase() &&
                <p className="text-xs text-yellow-300 mt-2">Only this election's admin can add candidates.</p>}
            </section>

            {/* Authorize Voter Section */}
            <section>
              <h4 className="text-base sm:text-lg font-medium mb-2 text-white">Authorize a Voter</h4>
              <form onSubmit={handleAuthorizeVoter} className="flex flex-col sm:flex-row gap-2 sm:gap-3">
                <input
                  type="text"
                ...
                <button
                  type="submit"
                  disabled={isLoading || currentElectionDetails.isActive || currentElectionDetails.isCompleted || currentElectionDetails.admin.toLowerCase() !== account.toLowerCase()}
                  className="px-4 py-2 sm:px-6 sm:py-3 bg-yellow-500 hover:bg-yellow-600 rounded-lg text-white font-semibold transition duration-150 ease-in-out disabled:opacity-50 disabled:cursor-not-allowed text-sm sm:text-base"
                >
                  Authorize Voter
                </button>
              </form>
              {(currentElectionDetails.isActive || currentElectionDetails.isCompleted) &&
                <p className="text-xs text-red-300 mt-2">Cannot authorize voters for an active or completed election.</p>}
              {currentElectionDetails.admin.toLowerCase() !== account.toLowerCase() &&
                <p className="text-xs text-yellow-300 mt-2">Only this election's admin can authorize voters.</p>}
            </section>

            {/* Start/End Election Buttons */}
            <section className="flex flex-wrap gap-3 sm:gap-4 items-center">
              <h4 className="text-base sm:text-lg font-medium text-white">Election Actions:</h4>
              {currentElectionDetails.admin.toLowerCase() === account.toLowerCase() ? (
                <>
                  {!currentElectionDetails.isActive && !currentElectionDetails.isCompleted && (
                    <button
                      ...
                      disabled={isLoading}
                      className="px-4 py-2 sm:px-6 sm:py-3 bg-indigo-500 hover:bg-indigo-600 rounded-lg text-white font-semibold transition duration-150 ease-in-out disabled:opacity-50 disabled:cursor-not-allowed text-sm sm:text-base"
                    >
                      Start Election
                    </button>
                  )}
                  {currentElectionDetails.isActive && !currentElectionDetails.isCompleted && (
                    <button
                      ...
                      disabled={isLoading}
                      className="px-4 py-2 sm:px-6 sm:py-3 bg-red-500 hover:bg-red-600 rounded-lg text-white font-semibold transition duration-150 ease-in-out disabled:opacity-50 disabled:cursor-not-allowed text-sm sm:text-base"
                    >
                      End Election
                    </button>
                  )}
                  {currentElectionDetails.isCompleted && (
                    <span className="text-green-300 font-semibold text-sm sm:text-base">Election Concluded.</span>
                  )}
                </>
              ) : (
                <p className="text-xs sm:text-sm text-gray-400">You are not the administrator for this election. Only the election admin can start/end it.</p>
              )}
            </section>
          </div>
        </>
